...
const wait = ref()
const dataPegador = ref([])
async function getApi(){
  wait.value = true
  const {data:pegador} = await useFetch(`/api/routes/pegador`)
wait.value = false
dataPegador.value = pegador.value.data;
store.reloadData = pegador.value.data;
store.queryItems = dataPegador.value;
...
<template>
  ...
  <main class="p-6 min-h-screen">
    <h1 class="text-[3.5rem] font-semibold text-blue-950 uppercase">Pegador</h1>
